...
export const listUsers = async (req: any, res: Response) => {
  try {
    const usersResult = await pool.query(
      "SELECT id, email, name, is_platform_admin as \"isAdmin\", created_at FROM users ORDER BY created_at ASC",
    );
    ...
    for (const m of membershipsResult.rows) {
      if (req.user && !membershipsByUser[m.user_id])
    ...
    logger.debug(
      { userId: req.user.id, count: users.length },
      "Users and memberships listed",
    );
    res.json({ users });
  } catch (err: any) {
    logger.error(
...
export const createUser = async (req: any, res: Response) => {
  ...
  logger.info(
    { adminId: req.user.id, targetEmail: email, isAdmin },
  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO users (email, name, password_hash, is_platform_admin)
       VALUES ($1, $2, $3, $4)
       RETURNING id, email, name, is_platform_admin as "isAdmin", created_at`,
      [email, name, password_hash, isAdmin || false],
    );

    const user = result.rows[0];
    logger.info(
      { adminId: req.user.id, userId: user.id, email: user.email },
    ...
    if (err.code === "23505") {
      logger.warn(
        { adminId: req.user.id, targetEmail: email },
        "User creation failed: Email already exists",
      );
      return res.status(409).json({ error: "Email already registered." });
    }
    logger.error(
      { err: err.message, adminId: req.user.id },
...
export const deleteUser = async (req: any, res: Response) => {
  ...
  try {
    ...
    logger.error(
      { err: err.message, adminId: req.user.id, targetUserId: id },
...
export const revokeAllAccess = async (req: any, res: Response) => {
  ...
  try {
    const result = await pool.query(
      "DELETE FROM project_members WHERE user_id = $1 RETURNING *",
      [id],
    );
    logAudit(req.user.id, null, "user_global_revoke", {
      targetUserId: id,
      revokedCount: result.rowCount,
    });
    logger.info(
      { adminId: req.user.id, targetUserId: id, revokedCount: result.rowCount },
      "Global access revocation triggered",
    );
    res.json({ success: true, count: result.rowCount });
  } catch (err: any) {
    logger.error(
      { err: err.message, adminId: req.user.id, targetUserId: id },
      "Revocation process failed",
    );
    res.status(500).json({ error: "Internal server error." });
  }
};
...
export const changePassword = async (req: any, res: Response) => {
  ...
  try {
    ...
    logger.error(
      { err: err.message, userId: id },
      "Password change failed internally",
    );
    res.status(500).json({ error: "Internal server error." });
  }
};

// GET /users/search?query=... — search by email or name (any authenticated user)
export const searchUsers = async (req: any, res: Response) => {
  ...
  try {
    const result = await pool.query(
      `SELECT id, email, name, is_platform_admin as "isAdmin" FROM users 
    ...
    logger.error({ err: err.message, query }, "User search failed");
    res.status(500).json({ error: "Internal server error." });
  }
};
